...
// This plugin registers its /editor channel via connection.register(webCtx,
// ...) directly, not the connection.rpc.handle(...) convenience getter -
// see the comment at that call site for why. webServer stays in this static
// inject list so activation waits for it to exist in the tree at all, even
// though the actual property access happens through a nested ctx.inject().
...
export function apply(ctx) {
  ...
  // dsh-client-connection@0.1.5-alpha.1's own `rpc.handle` convenience getter
  // hardcodes `owner = this.ctx` to the Connection service's own internal ctx
  // (bound once, inside dsh-client-connection's own apply()) - a ctx that
  // never itself received webServer via inject, so any consumer's rpc.handle()
  // call throws "cannot get property webServer without inject" regardless of
  // what this plugin's own inject list declares (reproduced directly: still
  // failed identically even after confirming, via a real symlink, that
  // @deepseek-ai/cordis itself is a single shared instance - not a duplicate-
  // module-instance problem at all). register(owner, channel, handler) is the
  // exact same public method rpc.handle delegates to, but takes the owning
  // ctx as an explicit parameter - call it directly with a ctx that really
  // does have webServer, bypassing only the one broken getter and reusing
  // dsh-client-connection's own wire protocol untouched.
  ctx.inject(['webServer'], (webCtx) => {
    ctx.effect(() => ctx.connection.register(webCtx, '/editor', async (endpoint, payload, signal) => {
